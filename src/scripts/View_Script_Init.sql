...
as
  (
  select id, code, userId, userName, userGroup, date, dayofweek(date) as diaSemanaPunch, device, deviceId, userDefinedSchedulerId, schedulerId, userDefinedSchedulerName, exception,
    shiftSupposedTimeIn, shiftSupposedTimeOut, shiftSupposedGracePerior, json, punchType, shiftSupposedTimeIn as timeIn, shiftSupposedTimeOut as timeOut, minTimeIn,
...
as
  ...
  from (
    ...
    left outer join
    (
	 select shift.*, userShift.userId, userShift.shiftId, userShift.groupId, userShift.groupName, userShift.dateStart, userShift.dateEnd
    from userShift inner join shift on userShift.shiftId = shift.id
...
date, userId, userName, userGroup, shiftId, description, entrada, entradashift, totalHorasEntrada, totalHorasSaida, SEC_TO_TIME
(totalHorasSaida - totalHorasEntrada) totalHorasTrabalho,
...
select subtime(time(tab1.entrada), time(entradashift)) delayEntrance, subtime( time(saida), time(tab1.saidashift)) delayOut,
  tab1.*
from ( select v.date, v.userid, v.userName, userGroup, v.shiftid, v.description, min(v.entrada) entrada, Sum(Time_to_sec(time(v.entrada))) totalHorasEntrada, Sum(Time_to_sec(time(v.saida))) totalHorasSaida,
...
as
  ...
  from (
    select userId, min(userName) userName, max(userGroup) userGroupId, count(date) workingDays, SEC_TO_TIME(Sum(Time_to_sec(totalDelay))) totalDelay, SEC_TO_TIME(Sum(Time_to_sec(totalHorasTrabalho))) totalWorkingHours
...
BEGIN
  ...
  from (select tab1.*, userGroup.name userGroup
    ...
    select userId, min(userName) userName, max(userGroup) userGroupId, count(date) workingDays,
        SEC_TO_TIME(Sum(Time_to_sec(totalDelay))) totalDelay, SEC_TO_TIME(Sum(Time_to_sec(totalHorasTrabalho))) totalWorkingHours
      from view_PunchDaily
